...
import {
    ...
    NumberInputField,
    NumberInputStepper,
    Stack, Stat, StatHelpText, StatLabel, StatNumber,
    Text,
    useColorModeValue,
    useToast
...
const DepositForm = ({submitFormCallback, pairInfo, recipient, myBalance}: {
    ...
    const handlerCalculateValue = (e: any) => {
        ...
        if (pairInfo && getValues) {
            // @ts-ignore
            setCalculateValue(Number(Number(getValues()!.amount) / (Number(pairInfo!.value) / (10 ** 5))).toFixed(10))
        }
    }
                ...
                <Stack>
                    {(account?.address !== recipient)
                        ? <form onSubmit={handleSubmit(onSubmitDeposit)}>
                            ...
                            {isCalculating &&
                                <Flex w={'100%'} style={{marginTop: 10}} align={'center'} justify={'space-between'}>
                                    <Box
                                        w={'100%'}
                                        bg={useColorModeValue('white', 'gray.800')}
                                        boxShadow={'md'}
